...
export default function Dashboard() {
  ...
  const ticketStats = useMemo(() => {
    ...
    const spent = tickets.reduce((sum, t) => sum + (t.amountPaid ?? t.originalPurchasePrice ?? t.eventId?.price ?? 0), 0);
    ...
    return (
      <div className={`bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl shadow-md hover:shadow-lg hover:shadow-[#FFA500]/20 transition w-full overflow-hidden relative ${isUsed ? 'opacity-60 grayscale' : ''}`}>
        ...
        <div className="p-4">
          ...
          <p className="text-[#FFA500] font-medium mt-2 text-sm">Price: Rs {ticket.amountPaid ?? ticket.originalPurchasePrice ?? event.price}</p>
          {ticket.isForResale && (
